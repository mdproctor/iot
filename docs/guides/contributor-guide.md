...
| `mcp` | `casehub-iot-mcp` | -- | `IoTDeviceMcpTool @ApplicationScoped` -- 4 MCP tools (`iot_get_devices`, `iot_get_state`, `iot_send_command`, `iot_get_history`). Library module. Host-agnostic: injects `DeviceRegistry`, `Instance<DeviceProvider>`, `Instance<DeviceStateHistoryProvider>`, `Event<IoTCommandAuditEvent>`. All methods annotated `@Blocking`. |
